...
**POST with /ows and data query parameter:**

```sh
curl -v -X POST --data "service=wms" "http:///localhost:8222/geo/ows"
```
...
curl -v -X POST --data "$DATA&service=wms" "http:///localhost:8222/geo/ows"
```
