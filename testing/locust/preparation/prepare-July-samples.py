...
def generic_report_for_key(key, filename, cutoff, quantity):
    ...
    for item in entries:
        ...
        if (item['request'] == 'GETMAP' or item['request'] == 'GETCAPABILITIES'):
            hour = item['time'].hour
            min = item['time'].minute
            tm = (hour * 60) + min
            ## only get entries between 8:00PM and 9:00PM
            # if tm >= (19*60) and tm <= (20*60):
            scope = scope + 1
            item_copy = dict(item)
            item_copy.pop('time', None)
            new_entries.append(item_copy)
                ## unique requests
                # key = item['query']
    ...
    # new_entries = filter(new_entries)
    # lst = count_value(new_entries, key)
    ...
    f = open("data/July-23-2024.txt", "w")
